...
df <- t(read.delim('Hdistr_fix.txt',row.names=1, header=FALSE))

ss_df <- df[,c("ss_None","ss_Low","ss_Medium","ss_High")]
#nss_df <- df[,c("nss_None","nss_Low","nss_Medium","nss_High")]

df2 <- t(read.delim('Ldistr_fix.txt',row.names=1, header=FALSE))
...
pdf("MLdistr_fix.pdf", width=10, height=4, family="Arial", useDingbats = FALSE)
