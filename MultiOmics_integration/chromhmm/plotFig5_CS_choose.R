...
pdf("NannoEpi_Fig5CS_small.pdf", useDingbats=FALSE, width=6, height=4, family="Arial")
...
df$CS_CS <- paste0(df$CS1,"_",df$CS2)
df_real$CS_CS <- paste0(df_real$CS1,"_",df_real$CS2)

choosen <- c("CS4_CS1","CS4_CS3","CS4_CS5","CS5_CS2","CS5_CS4")

df2 <- df[df$CS_CS %in% choosen,]
df_real2 <- df_real[df_real$CS_CS %in% choosen,]

str(df2)
str(df_real2)

ggboxplot(df2, x="CS_CS", y="Percent", group="CS_LC", color="CS_LC", palette="jco",
              size=0.2, bxp.errorbar=T, facet.by="Type", ncol=2) + 
  geom_point(data=df_real2, aes(shape=Type), fill="red", colour="red", size=2, 
