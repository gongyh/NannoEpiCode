...
if (file.exists("diffExpr.P0.001_C1.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("diffExpr.P0.001_C1.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("diffExpr.P0.001_C1.matrix", header=T, com='', sep="\t", row.names=1, check.names=F)
...
write.table(data, file="diffExpr.P0.001_C1.matrix.log2.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="diffExpr.P0.001_C1.matrix.log2.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("diffExpr.P0.001_C1.matrix.log2.sample_cor_matrix.pdf")
...
", "diffExpr.P0.001_C1.matrix.log2") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
...
write.table(data, file="diffExpr.P0.001_C1.matrix.log2.centered.dat", quote=F, sep='	');
...
hc_genes = hclust(gene_dist, method='complete')
heatmap_data = data
pdf("diffExpr.P0.001_C1.matrix.log2.centered.genes_vs_samples_heatmap.pdf")
...
", "diffExpr.P0.001_C1.matrix.log2.centered" ) , ColSideColors=sampleAnnotations)
dev.off()
save(list=ls(all=TRUE), file="diffExpr.P0.001_C1.matrix.RData")
