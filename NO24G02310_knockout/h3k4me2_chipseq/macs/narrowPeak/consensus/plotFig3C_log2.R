...
pdf("NannoEpi_M4M6_log2.pdf", useDingbats=FALSE, width=4, height=4, family="Arial")

df$ratio <- log2(df$ratio + 1e-10)
df2$ratio <- log2(df2$ratio + 1e-10)

ggboxplot(df, x="histone", y="ratio", fill="updn", palette="jco", size=0.2, bxp.errorbar=T, outlier.shape=NA) + coord_cartesian(ylim=c(0,5)) +
