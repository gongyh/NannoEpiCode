...
pdf("NannoEpi_Fig2_H0_log10.pdf", useDingbats = FALSE, width = 8, height = 6, family="Arial")

comparisons <- list(c("cluster_4","cluster_5"),c("cluster_3","cluster_4"),
                    c("cluster_2","cluster_3"),c("cluster_1","cluster_2"))

p1 <- ggboxplot(H0_H3k9ac_df,x="Group",y="Control",fill="Group",palette="npg", width=0.5, add="none", bxp.errorbar=T, 
  ...
  xlab("Clusters of H3k9ac (H0)") + ylab("Gene expression (TPM+1, log10)") + 
...
p2 <- ggboxplot(H0_H3k27ac_df,x="Group",y="Control",fill="Group",palette="npg", width=0.5,
  ...
  theme(text=element_text(size=11, family="Arial"), legend.position="none",
        ...
        axis.text.x = element_text(color="black"),
        axis.text.y = element_text(color="black")) +
  xlab("Clusters of H3k27ac (H0)") + ylab("Gene expression (TPM+1, log10)") + 
  stat_compare_means(aes(label = ..p.signif..), method="wilcox.test", comparisons=comparisons, family="Arial")
...
p3 <- ggboxplot(H0_H3kcr_df,x="Group",y="Control",fill="Group",palette="npg", width=0.5,
  ...
  theme(text=element_text(size=11, family="Arial"), legend.position="none",
        ...
        axis.text.x = element_text(color="black"),
        axis.text.y = element_text(color="black")) +
  xlab("Clusters of H3kcr (H0)") + ylab("Gene expression (TPM+1, log10)") + 
  stat_compare_means(aes(label = ..p.signif..), method="wilcox.test", comparisons=comparisons, family="Arial")
...
p4 <- ggboxplot(H0_H3k4m2_df,x="Group",y="Control",fill="Group",palette="npg", width=0.5,
  ...
  theme(text=element_text(size=11, family="Arial"), legend.position="none",
        ...
        axis.text.x = element_text(color="black"),
        axis.text.y = element_text(color="black")) +
  xlab("Clusters of H3k4m2 (H0)") + ylab("Gene expression (TPM+1, log10)") + 
  stat_compare_means(aes(label = ..p.signif..), method="wilcox.test", comparisons=comparisons, family="Arial")
