...
pdf("NannoEpi_Fig5CS_bar.pdf", useDingbats=FALSE, width=6, height=4, family="Arial")
...
ggbarplot(df_real, x="CS_CS", y="Percent", fill="Type", color="Type", palette="jco", 
  ...
  geom_text(aes(label = stars.pval(pvalue)), vjust=0,
          position=position_dodge2(width=0.9, preserve="single", padding=0)) +
  theme_bw() + 
  theme(text=element_text(size=14, family = "Arial"), legend.position="top",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        axis.text.x = element_text(color="black",angle=20,hjust=1),
