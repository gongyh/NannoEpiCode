...
pdf("GO_CS5CS4_bar.pdf", useDingbats = FALSE, width = 5, height = 3, family="Arial")
...
dupGOs <- c("GO:0043232", "GO:0034645", "GO:0006518")
...
ggbarplot(data, x="ID", y="qvalue", fill="Ontology", color="Ontology", group="Ontology", sort.val="desc",
          ...
          scale_y_continuous(trans=reverselog_trans(10),limits=c(1,1e-8), expand = c(0, 0.1),
                             labels = trans_format("log10", math_format(10^.x))) +
    geom_text(aes(y=1,label=Description), hjust = 0, vjust=-1, family="Arial", size = 3)+
    geom_text(aes(y=5e-8,label=Count), hjust = -0.5, vjust=0.5, family="Arial", size = 3)+
...
pdf("GO_CS5CS4_upset.pdf", useDingbats = FALSE, width = 5, height = 4, family="Arial")
