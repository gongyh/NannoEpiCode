...
pdf("NannoEpi_HiC_AB_chr14_log2fc.pdf", useDingbats=FALSE, width=5, height=5, family="Arial", onefile=F)

HC_data <- HC_R1_df[HC_R1_df$V1=="chr14",]
LC_data <- LC_R1_df[LC_R1_df$V1=="chr14",]

bgl_data <- bin_gene_log2fc[bin_gene_log2fc$V1=="chr14",]
#str(bgl_data)
...
a <- ggdotchart(data, x="pos", y="V4", sorting="none", add = "segment", add.params=list(size=0.5,color="sign"),
  ...
  xlab("Chromosome 14") + ylab("Eigenvector")
...
ab <- ggbarplot(ab24_df, x="x", y="value", group="sign", fill="sign", palette=c("blue","red"), 
  ...
  scale_x_discrete(labels=c("HC_TRUE"="HC_A","HC_FALSE"="HC_B","LC_TRUE"="LC_A","LC_FALSE"="LC_B")) + theme_bw() +
  theme(text=element_text(size=11, family="Arial"), legend.position="none",
        panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
  ...
  stat_compare_means(comparisons=cmps, method="t.test", 
                     label.y=c(300,120), tip.length=0.003)
...
b <- ggboxplot(data3, x="type", y="V2.y", notch=T, outlier.shape=NA, bxp.errorbar=T) +
